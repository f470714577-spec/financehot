...
> 最近核对：2026-08-16；安装器与镜像地址属于外部依赖，使用前如失效应以 Docker 官方说明为准。
...
若首次 `docker compose up` 卡在 `Pulling` 并报连接超时，可为 Docker Desktop 配置 registry mirror；镜像站可用性会变化，以下仅为项目历史验证过的候选：
...
完成环境验证后，按 `docs/onboarding.md` 拉起项目，并以 `PROJECT_CONTEXT.md` 的“下一阶段”为准继续开发。阶段 02 已完成，不要从旧阶段重复开始。
