...
- 当前目录仅含开发总控包 docx 及 Word 锁文件；无 git、无代码、无配置、无 PROJECT_CONTEXT.md。
- **结论：按从零项目处理。** 阶段 01 完成 git init、Monorepo 骨架与开发环境。
...
│   ├── ai/             # LLMProvider 抽象、prompt、JSON 校验（仅依赖 shared）
│   ├── crawler/        # SourceAdapter + RSS/API/Web 实现 + SSRF 防护（仅依赖 shared）
│   └── ui/             # Design Tokens、基础组件（依赖 shared）
├── prompts/  scripts/  docker/  docs/
...
`crawl / normalize / ai_process / embedding / cluster / score / daily_report`（未实现的 job 保留 handler stub）。
