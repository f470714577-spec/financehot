...
- 项目锁定 `pnpm@11.21.0`；本机没有 `corepack`，已在用户级 npm 目录安装同版本 pnpm。`pnpm --version` 返回 `11.21.0`，标准 `pnpm lint/typecheck/test/build` 命令可直接执行。
...
截图目录只保留上述 8 张实际 JPEG 文件；验收修正时已删除 8 个内容完全相同且扩展名错误的 `.png` 副本。
