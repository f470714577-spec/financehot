...
- 顺序：候选与 Event 事实更新 → 事务化 merge/split → Event API/前台 → 反向红绿 → 文档与最多3轮根级门禁。
- 最大风险：同公司不同事件误合并；其次是共享 PostgreSQL/Redis 测试隔离、边界 LLM 无配置保守新建和事务幂等。
- 约束依据：沿用阶段09现有 Embedding/关系/幂等实现，只在白名单内做最小增量；不改 Seed、旧 migration、阈值测试，不用真实 Key，不 push/deploy。

## 阶段10任务1–3完成回执（2026-08-19）
- 任务1完成：候选保护集中组合 72 小时窗口、当前向量元数据、分类、国家实体、标题特征和动作冲突；高置信直接合并，边界才调用结构化 LLM，失败/未配置保守新建；Event 标题、摘要、计数与首末时间按成员事实事务内重算，自动状态不编造 `confirmed`。
- 任务1真实 PostgreSQL/Redis 测试覆盖 5 信源同事实单 Event、同日同公司不同事件分离、低相似/分类冲突隔离、增量计数、无配置零请求、边界单次请求、缓存重放无新增调用/任务/向量/关系；Worker 由基线 42 项增至 45 项。
- 任务2完成：`event-facts.ts` 与 `event-corrections.ts` 提供事务化、幂等 merge/split；`event_articles` 是唯一关系事实源，唯一主报道、计数、首末时间同事务维护；真实 PostgreSQL 测试覆盖成功、重复、并发、非法输入回滚和明确成员 split，无需新增 migration。
